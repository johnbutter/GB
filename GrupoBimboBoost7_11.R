...
median_train <- fread("~/Desktop/Data/Kaggle/GrupoBimbo/train.csv",select = c('Agencia_ID', 'Cliente_ID', 'Producto_ID', 'Demanda_uni_equil')
...
clients<-subset(completeData, Demanda_uni_equil<1000 &Demanda_uni_equil>1)
...
median_Client_Prod <- median_train[, exp(median(log(Demanda_uni_equil+1)))*0.8+0.1,by = .(Producto_ID, Cliente_ID)]
...
#calculate new demand prediction (1/2 * the difference between XGB and Median added to XGB)
ensemble$NewDemand<-ensemble$Demanda_uni_equil+((ensemble$mpred-ensemble$Demanda_uni_equil)/2)
