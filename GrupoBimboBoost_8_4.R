...
cat("Product-Client pairs with orders in multiple weeks:", sum(feature_PC$Mult_Wks)/nrow(train)*100,"%\n");
...
train<- selected_range %>% sample_frac(0.02) #settled on .1-10% for the model build
...
##### Temporary Remove - Use to Test Single interations
...
for (iter in 1:nloops) {
  param <- list(objective = "reg:linear",
                max_depth = sample(8:11, 1), #8 produced a 0.53
