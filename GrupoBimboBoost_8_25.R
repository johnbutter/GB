...
setkey(xgb_train,Cliente_ID,Semana)
feature_CW<-xgb_train[,list(
                            C_total=sum(Demanda_uni_equil),
                            C_mean=mean(Demanda_uni_equil),
                            C_median=as.numeric(median(Demanda_uni_equil))),
                      by=.(Cliente_ID, Semana)
                      ] 
setkey(xgb_train,Ruta_SAK, Semana)
feature_RW<-xgb_train[,list(
  R_total=sum(Demanda_uni_equil),
  R_mean=mean(Demanda_uni_equil),
  R_median=as.numeric(median(Demanda_uni_equil))),
  by=.(Ruta_SAK, Semana)
  ] 

setkey(xgb_train,Producto_ID, Ruta_SAK, Semana)
feature_PRW<-xgb_train[,list(
  PR_total=sum(Demanda_uni_equil),
  PR_mean=mean(Demanda_uni_equil),
  PR_median=as.numeric(median(Demanda_uni_equil))),
  by=.(Producto_ID, Ruta_SAK, Semana)
  ] 
...
write.csv(data.frame(feature_CW), "~/Desktop/Data/Kaggle/GrupoBimbo/all_feature_CW.csv", row.names = F)
write.csv(data.frame(feature_PRW_summary), "~/Desktop/Data/Kaggle/GrupoBimbo/all_feature_PRW_summary.csv", row.names = F)
write.csv(data.frame(feature_RW_summary), "~/Desktop/Data/Kaggle/GrupoBimbo/all_feature_RW_summary.csv", row.names = F)
...
train<- selected_range %>% sample_frac(0.05) #settled on .1-10% for the model build
...
setkey(feature_PCW_summary,"Producto_ID", "Cliente_ID", "Semana")
setkey(feature_PC,"Producto_ID", "Cliente_ID")
setkey(feature_PRW,"Producto_ID", "Ruta_SAK", "Semana")
setkey(feature_CW,"Cliente_ID", "Semana")
setkey(feature_RW,"Ruta_SAK", "Semana")
...
## Add Previous 1 Week, Client Medians
setnames(feature_CW, c("C_total","C_mean","C_median"),c("C1_total","C1_mean","C1_median"))
setkey(ohetrain, "Cliente_ID", "Prev1")
setkey(ohetest, "Cliente_ID", "Prev1")
ohetrain <- merge(ohetrain, feature_CW, by.x = c("Cliente_ID", "Prev1"), by.y= c("Cliente_ID", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_CW, by.x = c("Cliente_ID", "Prev1"), by.y= c("Cliente_ID", "Semana"), all.x = TRUE)

## Add Previous 2 Week, Client Medians
setnames(feature_CW, c("C1_total","C1_mean","C1_median"),c("C2_total","C2_mean","C2_median"))
setkey(ohetrain, "Cliente_ID", "Prev2")
setkey(ohetest, "Cliente_ID", "Prev2")
ohetrain <- merge(ohetrain, feature_CW, by.x = c("Cliente_ID", "Prev2"), by.y= c("Cliente_ID", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_CW, by.x = c("Cliente_ID", "Prev2"), by.y= c("Cliente_ID", "Semana"), all.x = TRUE)

## Add Previous 3 Week, Client Medians
setnames(feature_CW, c("C2_total","C2_mean","C2_median"),c("C3_total","C3_mean","C3_median"))
setkey(ohetrain, "Cliente_ID", "Prev3")
setkey(ohetest, "Cliente_ID", "Prev3")
ohetrain <- merge(ohetrain, feature_CW, by.x = c("Cliente_ID", "Prev3"), by.y= c("Cliente_ID", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_CW, by.x = c("Cliente_ID", "Prev3"), by.y= c("Cliente_ID", "Semana"), all.x = TRUE)
setnames(feature_CW, c("C3_total","C3_mean","C3_median"),c("C_total","C_mean","C_median"))


## Add Previous 1 Week, Ruta Medians
setnames(feature_RW, c("R_total","R_mean","R_median"),c("R1_total","R1_mean","R1_median"))
setkey(ohetrain, "Ruta_SAK", "Prev1")
setkey(ohetest, "Ruta_SAK", "Prev1")
ohetrain <- merge(ohetrain, feature_RW, by.x = c("Ruta_SAK", "Prev1"), by.y= c("Ruta_SAK", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_RW, by.x = c("Ruta_SAK", "Prev1"), by.y= c("Ruta_SAK", "Semana"), all.x = TRUE)

## Add Previous 2 Week, Ruta Medians
setnames(feature_RW, c("R1_total","R1_mean","R1_median"),c("R2_total","R2_mean","R2_median"))
setkey(ohetrain, "Ruta_SAK", "Prev2")
setkey(ohetest, "Ruta_SAK", "Prev2")
ohetrain <- merge(ohetrain, feature_RW, by.x = c("Ruta_SAK", "Prev2"), by.y= c("Ruta_SAK", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_RW, by.x = c("Ruta_SAK", "Prev2"), by.y= c("Ruta_SAK", "Semana"), all.x = TRUE)

## Add Previous 3 Week, Ruta Medians
setnames(feature_RW, c("R2_total","R2_mean","R2_median"),c("R3_total","R3_mean","R3_median"))
setkey(ohetrain, "Ruta_SAK", "Prev3")
setkey(ohetest, "Ruta_SAK", "Prev3")
ohetrain <- merge(ohetrain, feature_RW, by.x = c("Ruta_SAK", "Prev3"), by.y= c("Ruta_SAK", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_RW, by.x = c("Ruta_SAK", "Prev3"), by.y= c("Ruta_SAK", "Semana"), all.x = TRUE)
setnames(feature_RW, c("R3_total","R3_mean","R3_median"),c("R_total","R_mean","R_median"))


## Add Previous 1 Week, P-Ruta Medians
setnames(feature_PRW, c("PR_total","PR_mean","PR_median"),c("PR1_total","PR1_mean","PR1_median"))
setkey(ohetrain, "Producto_ID","Ruta_SAK", "Prev1")
setkey(ohetest,"Producto_ID", "Ruta_SAK", "Prev1")
ohetrain <- merge(ohetrain, feature_PRW, by.x = c("Producto_ID","Ruta_SAK", "Prev1"), by.y= c("Producto_ID","Ruta_SAK", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_PRW, by.x = c("Producto_ID","Ruta_SAK", "Prev1"), by.y= c("Producto_ID","Ruta_SAK", "Semana"), all.x = TRUE)

## Add Previous 2 Week, P-Ruta Medians
setnames(feature_PRW, c("PR1_total","PR1_mean","PR1_median"),c("PR2_total","PR2_mean","PR2_median"))
setkey(ohetrain, "Producto_ID","Ruta_SAK", "Prev2")
setkey(ohetest, "Producto_ID","Ruta_SAK", "Prev2")
ohetrain <- merge(ohetrain, feature_PRW, by.x = c("Producto_ID","Ruta_SAK", "Prev2"), by.y= c("Producto_ID","Ruta_SAK", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_PRW, by.x = c("Producto_ID","Ruta_SAK", "Prev2"), by.y= c("Producto_ID","Ruta_SAK", "Semana"), all.x = TRUE)

## Add Previous 3 Week, P-Ruta Medians
setnames(feature_PRW, c("PR2_total","PR2_mean","PR2_median"),c("PR3_total","PR3_mean","PR3_median"))
setkey(ohetrain, "Producto_ID","Ruta_SAK", "Prev3")
setkey(ohetest,"Producto_ID", "Ruta_SAK", "Prev3")
ohetrain <- merge(ohetrain, feature_PRW, by.x = c("Producto_ID","Ruta_SAK", "Prev3"), by.y= c("Producto_ID","Ruta_SAK", "Semana"), all.x = TRUE)
ohetest <- merge(ohetest, feature_PRW, by.x = c("Producto_ID","Ruta_SAK", "Prev3"), by.y= c("Producto_ID","Ruta_SAK", "Semana"), all.x = TRUE)
setnames(feature_PRW, c("PR3_total","PR3_mean","PR3_median"),c("PR_total","PR_mean","PR_median"))
...
# NEW FEATURE CORRECTIONS
ohetrain$C1_median[is.na(ohetrain$C1_median)] <- 0
ohetrain$C2_median[is.na(ohetrain$C2_median)] <- 0
ohetrain$C3_median[is.na(ohetrain$C3_median)] <- 0
ohetrain$R1_median[is.na(ohetrain$R1_median)] <- 0
ohetrain$R2_median[is.na(ohetrain$R2_median)] <- 0
ohetrain$R3_median[is.na(ohetrain$R3_median)] <- 0
ohetrain$PR1_median[is.na(ohetrain$PR1_median)] <- 0
ohetrain$PR2_median[is.na(ohetrain$PR2_median)] <- 0
ohetrain$PR3_median[is.na(ohetrain$PR3_median)] <- 0

ohetest$PR1_median[is.na(ohetest$PR1_median)] <- 0
ohetest$PR2_median[is.na(ohetest$PR2_median)] <- 0
ohetest$PR3_median[is.na(ohetest$PR3_median)] <- 0
ohetest$R1_median[is.na(ohetest$R1_median)] <- 0
ohetest$R2_median[is.na(ohetest$R2_median)] <- 0
ohetest$R3_median[is.na(ohetest$R3_median)] <- 0
ohetest$C1_median[is.na(ohetest$C1_median)] <- 0
ohetest$C2_median[is.na(ohetest$C2_median)] <- 0
ohetest$C3_median[is.na(ohetest$C3_median)] <- 0
...
ensemble$average.min<-(ensemble$hp3+ensemble$hp1+ensemble$hp2+ensemble$xgbp-ensemble$minp)/3
ensemble$average.max<-(ensemble$hp3+ensemble$hp1+ensemble$hp2+ensemble$xgbp-ensemble$maxp)/3
